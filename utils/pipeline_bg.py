...
def run_pipeline_thread(start_str, end_str, paths_config):
    ...
    try:
        ...
        set_pipeline_status({"step": 1, "message": f"Step 1: Acquired {len(df_mid)} new MIDs. Applying regex algorithms and merging with Master...", "status": "running"})
        new_n, tot_n = run_mid_cleaner(df_mid, PATH_MID, bkp_dir)

        set_pipeline_status({"step": 1, "message": "Step 1: Classification complete. Updating SQLite staging.db with new PROCESSED_MID...", "status": "running"})
